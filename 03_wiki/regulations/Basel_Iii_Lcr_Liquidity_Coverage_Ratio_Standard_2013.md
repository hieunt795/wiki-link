---
node_id: basel_iii_lcr_liquidity_coverage_ratio_standard_2013_001
type: regulation
jurisdiction: international
issuer: bcbs
title: Basel Iii Lcr Liquidity Coverage Ratio Standard 2013
aliases:
- LCR
- Liquidity Coverage Ratio
- bcbs238
- tỷ lệ đảm bảo thanh khoản
- Basel III LCR
- 30-day liquidity requirement
domain:
  primary: alm
tags:
- lcr
- hqla
- liquidity_risk
- bcbs
- basel_iii
- alm
- '2013'
confidence: 1
stability: evolving
thesis: '[LLM] Basel III''s LCR (bcbs238, Jan 2013) requires banks to maintain a stock
  of unencumbered HQLA sufficient to cover total net cash outflows (NCO) over 30 calendar
  days under a combined idiosyncratic and market-wide stress scenario: LCR = HQLA
  / NCO ≥ 100%. Phase-in: 60% from Jan 2015, rising 10 pp per year to 100% from Jan
  2019. The standard defines HQLA in two levels and prescribes standardised outflow
  rates by counterparty/product type.'
source_refs:
- path: 07_scripts/02_sources/regulator/bcbs/bcbs238.md
  pages: ''
  weight: primary
parent_node: null
related:
- Hqla_Classification_Level1_Level2a_Level2b
- Lcr_Monitoring_Tools_Maturity_Mismatch_Funding_Concentration
- Ilaap_Internal_Liquidity_Adequacy_Assessment_Framework
- Bcbs_Sound_Liquidity_Risk_Management_17_Principles_2008
date_created: '2026-05-27'
date_updated: '2026-05-27'
---

[LLM] Auto-generated stub from bcbs238.md. Review and expand.

