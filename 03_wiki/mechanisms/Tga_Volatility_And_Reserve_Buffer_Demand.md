...
This node is canonical for the qualitative channel from TGA volatility to reserve-buffer demand.

It should not repeat the TGA liability-swap accounting in full; that belongs to [[Treasury_General_Account_Tga_Reserve_Swap]].

It should not carry the full buffer formula; that belongs to [[Ample_Reserves_Buffer_Sizing_Tga_Volatility]].
...
That means the TGA is not just a Treasury bookkeeping account.

It is a reserve-management variable that can tighten or loosen money-market conditions through the Fed's balance sheet.
...
The level matters because a persistently higher Treasury cash balance requires the Fed to support a larger steady reserve base.

The volatility matters because abrupt swings in Treasury cash can consume the margin of safety the Fed relies on for smooth rate control.

In practical terms, level sets the baseline size of reserves, while volatility sets the amount of slack the system must carry above that baseline.
...
Those episodes are important because they can force fast reserve drains or injections that would otherwise have to be offset by active Fed operations.

The result is a tighter connection between Treasury cash management and the Fed's daily implementation work.

This also explains why reserve buffer design cannot be separated from Treasury issuance patterns, debt-ceiling constraints, or the Treasury's preferred cash-balance policy.

## Why It Matters
The key implication is not that the TGA mechanically determines the policy rate, but that it changes the probability of reserve scarcity at the margin.

A less volatile TGA would reduce the minimum reserve buffer needed for uninterrupted rate control.

A more volatile TGA raises the Fed's operating cost because the central bank must hold more slack or intervene more often.

This is a durable mechanism, even though the exact buffer size will change with payment-system structure, Treasury behavior, and the Fed's chosen operating framework.
