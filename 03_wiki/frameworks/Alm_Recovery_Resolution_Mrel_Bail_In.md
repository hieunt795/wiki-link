...
Under the BRRD framework, a bank that exhausts recovery options enters resolution via one of four tools (sale-of-business, bridge institution, asset separation, bail-in). [LLM] The bail-in tool absorbs losses by writing down or converting eligible MREL liabilities into equity; ALM is responsible for MREL forecasting in the funding plan, gone-concern ICAAP/ILAAP execution, and providing the resolution authority with resolvability evidence on capital, liquidity, and operational continuity. [LLM]

## Four Phases of a Bank Under Stress

Every bank can potentially undergo four sequential phases: [LLM]
...
A recovery plan defines: [LLM]
- **Recovery triggers**: Metrics (capital, liquidity, profitability) whose breach initiates escalation and option execution.
...
Banks must draft recovery plans themselves; resolution plans are drafted by the resolution authority with input from the bank. [LLM]

## Four BRRD Resolution Tools

When all recovery options are exhausted, authorities select one or a combination of: [LLM]
...
MREL instruments must be available to cover **loss absorption and recapitalization** needs in a resolution scenario. [LLM] ALM responsibilities: [LLM]
- **Forecasting**: Include MREL targets in the funding plan at each planning horizon.
...
MREL requirements are imposed on all banks subject to BRRD resolution strategy (not normal liquidation), i.e., systemically important institutions in Categories 1–3. [LLM] Smaller Category 4 banks default to normal insolvency — no MREL requirement. [LLM]

## Resolvability Areas

Supervisory authorities require banks to demonstrate resolvability in multiple areas: [LLM]
- **Financial**: Sufficient MREL instruments available; loss absorption and recapitalization capacity modeled.
...
When a bank enters stress, ALM executes a stacking order of liquidity actions: [LLM]

1. Reprice corporate/retail deposits downward to retain liquidity. [LLM]
2. Negotiate prolongation of interbank funding before counterparty limits close. [LLM]
3. Access central bank facilities (ECB TLTROs, central bank collateralized funding). [LLM]
4. Arrange intragroup liquidity support from parent company. [LLM]
5. Execute loan transfers to parent or third parties (true-sale or silent funded risk participation). [LLM]
6. As ultimate measure: accelerate asset repayment by clients (reputationally costly). [LLM]

## ALM Role in Crisis Capital Management

Capital action plan under stress: [LLM]
...
---
*Source: Box 14.1 and Chapter 15 of Bardaeva (ed.), "Bank Asset Liability Management Best Practice" (De Gruyter, 2021). All body sentences tagged [LLM] are synthesized from source material. Confidence: 1.*
