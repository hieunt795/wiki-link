...
# **Mục 1. QUY ĐỊNH CHUNG VỀ HOẠT ĐỘNG QUẢN LÝ RỦI RO Điều 17. Yêu cầu về quản lý rủi ro**
...
### **Chương IV** <sup>ơ</sup> **KIỂM TOÁN NỘI BỘ**
...
# **BÁO CÁO ĐÁNH GIÁ NỘI BỘ VỀ MỨC ĐỦ VỐN**
...
|------------------|----------------------------|--------|----------|----------|----------|
| CARtarget<br>(%) | Tỷ lệ an toàn vốn mục tiêu |        |          |          |          |
| Chỉ tiêu về      | ROE (%)                    |        |          |          |          |
| thu nhập         | RAROC (%)                  |        |          |          |          |
...
| TT | Vốn mục tiêu               | Đơn vị  | Năm<br>t0 | Năm<br>t0+1 | Năm<br>t0+2 | Năm<br>t0+3 |
|----|----------------------------|---------|-----------|-------------|-------------|-------------|
...
| 2   | ∆RWAB                                             | tỷ đồng |           |             |             |             |
| 3   | CARtarget                                         | %       |           |             |             |             |
| 4   | RWA*<br>CE<br>=<br>x<br>E                         | tỷ đồng |           |             |             |             |
|     | CARTarget + ∆RWAB x<br>CARR                       |         |           |             |             |             |
| 5   | CTarget<br>= Max (CR<br>,CE)                      | tỷ đồng |           |             |             |             |
| III | Chênh lệch giữa vốn tự có dự kiến và vốn mục tiêu |         |           |             |             |             |
| 1   | CA                                                | tỷ đồng |           |             |             |             |
| 2   | Chênh lệch                                        | tỷ đồng |           |             |             |             |
|     | = CA<br>-<br>CTarget                              |         |           |             |             |             |
...
- (i) 19 thang kỳ hạn có ngày định giá lại quy định tại Bảng 1 Phụ lục này,hoặc
- (ii) điểm giữa của thang kỳ hạn quy định tại Bảng 1 Phụ lục này. Trường hợp phân bổ dòng tiền vào điểm giữa của thang kỳ hạn, ngân hàng đảm bảo giữ nguyên kỳ hạn của dòng tiền này bằng việc chia nhỏ (splitting up) các dòng tiền giữa hai điểm giữa của thang kỳ hạn liền kề.
...
| STT | Thang kỳ hạn                | Điểm giữa thang kỳ hạn (𝒕𝒌) |
|-----|-----------------------------|-----------------------------|
...
|                                                                   | Giới hạn tỷ trọng<br>NMDs lõi (%) | Giới hạn kỳ hạn trung bình<br>NMDs lõi (năm) |
|-------------------------------------------------------------------|-----------------------------------|----------------------------------------------|
| NMDs cá nhân                                                      |                                   |                                              |
| NMDs lõi/ tổng NMDs trên tài<br>khoản giao dịch của cá nhân       | 90                                | 5                                            |
| NMDs lõi/ tổng NMDs trên tài<br>khoản không giao dịch của cá nhân | 70                                | 4,5                                          |
...
- Ngân hàng xác định <mark>tỷ lệ trả trước hạn cơ sở CPR<sup>p</sup><sub>0,c</sub></mark> (baseline conditional prepayment rate) cho mỗi danh mục p gồm các khoản cho vay theo đồng tiền c có cùng đặc điểm trong điều kiện hiện hành.
...
$$CPR_{i,c}^p = \min(1, \gamma_i \times CPR_{0,c}^p)$$

Trong đó:  $\gamma_i$  là hệ số nhân áp dụng cho kịch bản cú sốc lãi suất i theo Bảng 3 Phụ lục này

|                 | T                             | _          |
|-----------------|-------------------------------|------------|
| Số kịch bản (i) | Kịch bản cú sốc lãi suất      | $\gamma_i$ |
...
$$CF_{i,c}^{p}(k) = CF_{i,c}^{s}(k) + CPR_{i,c}^{p} * N_{i,c}^{p}(k-1)$$

Trong đó:

- $+ CF_{i,c}^{S}(k)$  là khoản trả gốc, lãi định kỳ.
- $+N_{i,c}^{p}(k-1)$  là số dư gốc còn lại của khoản vay tại thang kỳ hạn (k-1).
...
- Ngân hàng xác định tỷ lệ rút trước hạn cơ sở 0, (baseline term deposit redemption ratio) cho mỗi danh mục p bao gồm các khoản tiền gửi theo đồng tiền c có cùng đặc điểm trong điều kiện hiện hành.
- Ngân hàng xác định tỷ lệ rút trước hạn cho danh mục p này đối với thang kỳ hạn k hoặc điểm giữa thang kỳ hạn tktheo kịch bản cú sốc lãi suất i như sau:

$$TDRR_{i,c}^p = \min(1, u_i * TDRR_{0,c}^p)$$

**Bảng 4. Hệ số nhân theo các kịch bản cú sốc lãi suất**

| Số kịch bản | Kịch bản cú sốc lãi suất      | Hệ số nhân 𝒖𝒊 |
|-------------|-------------------------------|---------------|
...
$$CF_{i,c}^p(1) = TD_{0,c}^p * TDRR_{i,c}^p$$

Trong đó, 0, là số dư tiền gửi còn lại của danh mục p.
...
- Trong mỗi kịch bản, tất cả các dòng tiền được phân bổ vào thang kỳ hạn ∈ {1,2, …, } hoặc phân bổ vào điểm giữa của thang kỳ hạn , ∈ {1,2, …, }. Tại mỗi thang kỳ hạn k hoặc tại điểm giữa của thang kỳ hạn , ngân hàng xác định dòng tiền vào và dòng tiền ra của tài sản, nợ phải trả, khoản mục ngoại bảng nhạy cảm với lãi suất để tính dòng tiền ròng (dòng tiền vào trừ dòng tiền ra) cho từng thang kỳ hạn. Tổng kết quả các dòng tiền ròng tại tất cả các thang kỳ hạn là , () hoặc , ( ), ∈ {1,2, …, }.
...
$$DF_{i,c}(t_k) = \exp(-R_{i,c}(t_k) \times t_k)$$
...
$$EVE_{i,c}^{nao} = \sum_{K=1}^{K} CF_{i,c}(k) \times DF_{i,c}(t_k)$$

+ Trường hợp ngân hàng phân bổ dòng tiền vào điểm giữa của thang kỳ hạn:

$$EVE_{i,c}^{nao} = \sum_{K=1}^{K} CF_{i,c}(t_k) \times DF_{i,c}(t_k)$$
...
Ngân hàng tính ΔEVE đối với đồng tiền c theo kịch bản i bằng cách lấy chênh lệch giữa giá trị kinh tế của vốn chủ sở hữu hiện hành 0, với giá trị kinh tế của vốn chủ sở hữu theo kịch bản i , .

- Trường hợp ngân hàng phân bổ dòng tiền vào thang kỳ hạn có ngày định giá lại:

$$\Delta EVE_{i,c} = \sum_{K=1}^{K} CF_{0,c}(k) * DF_{0,c}(t_k) - \sum_{K=1}^{K} CF_{i,c}(k) * DF_{i,c}(t_k) + KAO_{i,c}(t_k)$$

- Trường hợp ngân hàng phân bổ dòng tiền vào điểm giữa của các thang kỳ hạn:

$$\Delta EVE_{i,c} = \sum_{K=1}^{K} CF_{0,c}(t_k) * DF_{0,c}(t_k) - \sum_{K=1}^{K} CF_{i,c}(t_k) * DF_{i,c}(t_k) + KAO_{i,c}(t_k)$$
...
$$\Delta EVE = \max_{i \in (1,2,\dots 6)} \left\{ max \left( 0; \sum_{C: \Delta EVE_{i,c} > 0} \Delta EVE_{i,c} \right) \right\}$$
...
# **C. Xác định cú sốc lãi suất** (∆ạ ố,) **cho các đồng tiền**

- **I.** Ngân hàng xác định cú sốc lãi suất cho các đồng tiền theo trình tự sau đây:
- 1. Ngân hàng áp dụng tham số ̅ạ ố, quy định tại Bảng 5 Phụ lục này trong tính toán cú sốc lãi suất cho đồng tiền c quy định tại khoản 3 Mục I Phần C Phụ lục này.
...
**Bảng 5. Tham số** ̅ạ ố, (đơn vị: bp)

|                                                                                                                                                                   | Dài hạn | 100 | 200 | 300 | 300 | 150 | 150 | 100 | 300 | 150 | 300 |
|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|-----|-----|-----|-----|-----|-----|-----|-----|-----|-----|
|                                                                                                                                                                   |         |     |     |     |     |     |     |     |     |     |     |
| 2. Trường hợp đồng tiền c không được quy định tại Bảng 5 Phụ lục này hoặc<br>ngân hàng không áp dụng tham số 𝑹̅<br>𝒍𝒐ạ𝒊 𝒔ố𝒄,𝒄 cho các đồng tiền quy định tại Bảng |         |     |     |     |     |     |     |     |     |     |     |
| 5 Phụ lục này, ngân hàng xác định tham số 𝑹̅<br>trong tính toán cú sốc lãi<br>𝒍𝒐ạ𝒊 𝒔ố𝒄,𝒄                                                                          |         |     |     |     |     |     |     |     |     |     |     |
...
suất cho đồng tiền c quy định tại khoản 3 Mục I Phần C Phụ lục này như sau:
...
b) Ngân hàng xác định tham số ̅ạ ố, bằng cách nhân lãi suất bình quân hằng ngày của đồng tiền c được xác định tại điểm a khoản này với tham số cú sốc lãi suất toàn cầu cơ sở (ˉ ) tương ứng theo Bảng 6 Phụ lục này.

**Bảng 6. Các tham số cú sốc lãi suất toàn cầu cơ sở** 

| Song song (parallel)           | 𝛼ˉ𝑝𝑎𝑟𝑎𝑙𝑙𝑒𝑙  | 60% |
|--------------------------------|-------------|-----|
| Lãi suất ngắn hạn (short rate) | 𝛼ˉ<br>𝑠ℎ𝑜𝑟𝑡 | 85% |
| Lãi suất dài hạn (long rate)   | 𝛼ˉ𝑙𝑜𝑛𝑔      | 40% |

- c) Tham số ̅ạ ố, quy định tại điểm b khoản này phải đảm bảo:
...
- 3. Ngân hàng xác định cú sốc lãi suất (∆ạ ố,) cho các kịch bản cú sốc lãi suất như sau:
...
Trong đó: 
$$S_{long}(t_k) = 1 - S_{short}(t_k) = (1 - e^{\frac{-t_k}{x}}); x = 4$$

d) Đối với kịch bản cú sốc dốc và kịch bản cú sốc phẳng cho đồng tiền c:

$$\Delta R_{steepener,c}(t_k) = -0.65 * \left| \Delta R_{short,c}(t_k) \right| + 0.9 * \left| \Delta R_{long,c}(t_k) \right|$$
  
$$\Delta R_{flattener,c}(t_k) = +0.8 * \left| \Delta R_{short,c}(t_k) \right| - 0.6 * \left| \Delta R_{long,c}(t_k) \right|.$$

**II.** Trường hợp ngân hàng không thực hiện quy định tại Mục I Phần C Phụ lục này, ngân hàng được tự xác định cú sốc lãi suất, đảm bảo phù hợp với hướng dẫn cập nhật của Ủy ban Basel về phương pháp xác định cú sốc lãi suất[1](#page-17-0) cho các kịch bản cú sốc lãi suất.

<span id="page-17-0"></span><sup>1</sup> Văn bản BCBS D578 - Bản hiệu chỉnh cú sốc lãi suất trên sổ ngân hàng năm 2024 (Recalibration of shocks in the interest rate risk in the banking book standard) của Ủy ban Basel.
...
$$C_E = RWA^*_E \times CAR_{Target} + \Delta RWA_B \times CAR_R$$
...
- d) **RWA\* <sup>E</sup>:** Tổng tài sản tính theo rủi ro trong kịch bản hoạt động bình thường được xác định theo công thức sau đây:

#### **RWA\* <sup>E</sup>= RWACR + RWAOR + RWAMR+ RWAIRRBB + RWACOR + RWAOMR**
...
$$RWA_{COR} = RWA^{1}_{COR} + RWA^{2}_{COR}$$

Trong đó:

- **RWA<sup>1</sup> COR**: Tổng tài sản tính theo rủi ro tập trung đối với hoạt động cấp tín dụng được xác định theo phương pháp của ngân hàng tự xây dựng nhưng không thấp hơn mức **RWA1\*COR** được xác định như sau:
  - + Công thức xác định:

**RWA**<sup>1\*</sup>COR = Max {
$$(E_i - A \times C); 0$$
} + Max { $(E_j - B \times C); 0$ }

Tỷ lệ vốn tự có tương ứng cho một khách hàng, một khách hàng và người có liên quan theo lộ trình sau:

| Từ         | Tỷ lệ đối với<br>một khách hàng (A) | Tỷ lệ đối với một khách hàng và<br>người có liên quan (B) |
|------------|-------------------------------------|-----------------------------------------------------------|
| 01/07/2026 | 8,7                                 | 16,8                                                      |
| 01/01/2027 | 8                                   | 15,2                                                      |
| 01/01/2028 | 7,3                                 | 13,6                                                      |
...
- + **RWA<sup>2</sup> COR**: Tổng tài sản tính theo rủi ro tập trung đối với hoạt động tự doanh được xác định theo phương pháp của ngân hàng tự xây dựng.
